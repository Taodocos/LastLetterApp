...
export interface PerformanceBGData {
  refNo: string;
  date: string;
  toName: string;
  address: string;
...
interface PerformanceBGFormProps {
  data: PerformanceBGData;
  onUpdate: (data: PerformanceBGData) => void;
}

const PerformanceBGForm: React.FC<PerformanceBGFormProps> = ({ data, onUpdate }) => {
  const [errors, setErrors] = useState<{ [key in keyof PerformanceBGData]?: string }>({});
  const handleChange = (field: keyof PerformanceBGData, value: string | number) => {
    onUpdate({ ...data, [field]: value });
    validateField(field, value); // Validate the field on change
  };

 const validateField = (field: keyof PerformanceBGData, value: string | number) => {
   ...
   const validatenumber = (field: keyof PerformanceBGData, value: string | number) => {
    const strValue = String(value); // Convert to string for validation
    if (strValue.trim() ==='' ) {
      setErrors((prev) => ({
        ...prev,
  ...
  const validateAllFields = () => {
    const newErrors: { [key in keyof PerformanceBGData]?: string } = {};
    
    for (const key in data) {
      const value = data[key as keyof PerformanceBGData];
      const strValue = String(value); // Convert to string for validation

      if (strValue.trim() === '' || strValue.length < 3) {
        newErrors[key as keyof PerformanceBGData] = `${key} is required and must be at least 3 characters long.`;
      }
    }
...
const calculateValidityDate = (fromDate: string, days: number) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
      <h2 className="text-xl font-semibold text-gray-900">Performance Bond Guarantee </h2>
...
export default PerformanceBGForm;