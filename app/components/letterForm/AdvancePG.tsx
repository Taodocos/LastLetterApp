...
export interface AdvancePGData {
  refNo: string;
  date: string;
  toName: string;
  address: string;
...
interface AdvancePGFormProps {
  data: AdvancePGData;
  onUpdate: (data: AdvancePGData) => void;
}

const AdvancePGForm: React.FC<AdvancePGFormProps> = ({ data, onUpdate }) => {
  const [errors, setErrors] = useState<{ [key in keyof AdvancePGData]?: string }>({});

  const handleChange = (field: keyof AdvancePGData, value: string | number) => {
    onUpdate({ ...data, [field]: value });
    validateField(field, value); // Validate the field on change
  };

 const validateField = (field: keyof AdvancePGData, value: string | number) => {
...
const calculateValidityDate = (fromDate: string, days: number) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
      <h2 className="text-xl font-semibold text-gray-900">Advance Payment Guarantee</h2>
