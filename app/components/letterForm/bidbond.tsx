...
export interface BidbondFormData {
  refNo: string;
  date: string;
  toName: string;
  address: string;
...
interface BidbondFormProps {
  data: BidbondFormData;
  onUpdate: (data: BidbondFormData) => void;
}

const BidbondForm: React.FC<BidbondFormProps> = ({ data, onUpdate }) => {
  const [errors, setErrors] = useState<{ [key in keyof BidbondFormData]?: string }>({});
   const handleChange = (field: keyof BidbondFormData, value: string | number) => {
     onUpdate({ ...data, [field]: value });
     validateField(field, value); // Validate the field on change
   };
 
  const validateField = (field: keyof BidbondFormData, value: string | number) => {
  ...
  const handleBondAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.replace(/,/g, ''); // Remove commas for parsing
    handleChange('bondAmount', value ? parseFloat(value) : 0); // Convert to number
  };

    const validatenumber = (field: keyof BidbondFormData, value: string | number) => {
     const strValue = String(value); // Convert to string for validation
     if (strValue.trim() ==='' ) {
       setErrors((prev) => ({
         ...prev,
...
const calculateValidityDate = (fromDate: string, days: number) => {
  ...
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
       ...
       <div className="space-y-2">
        ...
        <input
          ...
          value={formatNumber(data.bondAmount)} // Display formatted amount
          onChange={handleBondAmountChange} // Handle input changes
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        />
       ...
       <div className="space-y-4">
         ...
         {errors.numberOfDays && <p className="text-red-500">{errors.numberOfDays}</p>}
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               ...
               handleChange('bidNo', value ? parseFloat(value) : 0); // Convert to number or set to 0 if empty
               validatenumber('bidNo', value); // Validate the field on change
             }}
    ...
    {errors.bidDate && <p className="text-red-500">{errors.bidDate}</p>}
        </div>
               </div>
