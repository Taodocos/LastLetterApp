...
interface SuppliersBondPreviewProps {
  data: SuppliersBondData;
  qrCodeUrl: string;
}

const SuppliersBondPreview: React.FC<SuppliersBondPreviewProps> = ({
  data,
  qrCodeUrl,
}) => {
  ...
  return (
    ...
    >
      ...
      {qrCodeUrl ? (
        <div className="flex justify-end mb-4">
          <Image
            ...
            width={64}
            height={64}
            className="w-16 h-16" // Use className for styling
      ...
      <div className="text-center mb-6">
        <h2 className="text-lg font-bold">Suppliers Bond Guarantee</h2>
      </div>

      {/* Content */}
      <div className="space-y-4 text-justify leading-relaxed">
        <p>
          By order and for account of our client{" "}
         <span className="font-bold"> {capitalizeWords( data.clientName || "_______________________________________________")}{" "}</span>
          we hereby issue in your favor our Suppliers Bond Guarantee for the sum
          of <span className="font-bold"> {data.currency}{data.bondAmount ? formatNumber(data.bondAmount) : "_______________"} ({bondAmountInWords}) </span> to be valid up to{" "}
        <span className="font-bold"> {formatDate(data.validityDate || '___________')}  </span>
        </p>

        <p>
          The purpose of this Suppliers Bond Guarantee is to enable you to enter
          into a contract with the client for{" "}
         <span className="font-bold"> {data.contractPurpose || "_______________"}</span> as per your invitation to
          bid No <span className="font-bold">{data.bidNo || "_______________"} dated{" "}
          {formatDate(data.bidDate)}.</span>
        </p>

        <p>
          According to this Suppliers Bond Guarantee, we unconditionally
          undertake to pay you the sum not exceeding <span className="font-bold"> {data.currency }{" "}
          {data.bondAmount ? formatNumber(data.bondAmount) : "_______________"} (
          {bondAmountInWords}) </span> upon your written demand
         ...
         <span className="font-bold">{data.contractPurpose || "_______________"} </span>  as per contract and if
          your claim is presented to us within the validity date.
        </p>

        <p>
          However, this Suppliers Bond Guarantee will automatically become null
          and void just after the above validity date <span className="font-bold"> {formatDate(data.validityDate || '___________')}  </span> and any claim presented
...
export default SuppliersBondPreview;