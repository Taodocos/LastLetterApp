...
export interface AmharicLetter2Data {
  refNo: string;
  date: string;
  toName: string;
  address: string;
...
interface AmharicLetter2FormProps {
  data: AmharicLetter2Data;
  onUpdate: (data: AmharicLetter2Data) => void;
}

const AmharicLetter2Form: React.FC<AmharicLetter2FormProps> = ({ data, onUpdate }) => {
  const [errors, setErrors] = useState<{ [key in keyof AmharicLetter2Data]?: string }>({});

  const handleChange = (field: keyof AmharicLetter2Data, value: string | number) => {
  ...
  const validateField = (field: keyof AmharicLetter2Data, value: string | number) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="flex items-center space-x-2 text-sm font-medium text-gray-700">
            ...
            <span>የዋስትና ብር መጠን (Bond Amount)</span>
          </label>
          <input
            type="number"
...
export default AmharicLetter2Form;