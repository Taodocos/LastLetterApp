...
const AmharicLetterForm: React.FC<AmharicLetterFormProps> = ({ data, onUpdate }) => {
  const [errors, setErrors] = useState<{ [key in keyof AmharicLetterData]?: string }>({});

  const handleChange = (field: keyof AmharicLetterData, value: string | number) => {
  ...
  const validateField = (field: keyof AmharicLetterData, value: string | number) => {
